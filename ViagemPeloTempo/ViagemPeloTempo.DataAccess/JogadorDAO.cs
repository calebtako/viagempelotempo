...
namespace ViagemPeloTempo.DataAccess
{
    public class JogadorDAO
    {
        ...
        public void AlterarSenha(Jogador obj)
        {
            //Criando uma conexão com o banco de dados
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Db"].ConnectionString))
            {
                //Criando instrução sql para inserir na tabela de estados
                string strSQL = @"UPDATE jogador SET senha = @senha WHERE idjogador = @idjogador;";
                ...
                using (SqlCommand cmd = new SqlCommand(strSQL))
                {
                    cmd.Connection = conn;
                    //Preenchendo os parâmetros da instrução sql
                    cmd.Parameters.Add("@senha", SqlDbType.VarChar).Value = obj.NovaSenha;
